...
from app.util.create_desktop import get_desktop_kyb, get_tdektop
...
class DeAsyncModel:
    ...
    async def set_can_opc(self):
        ...
        if self.query.data == "off":
            data = get_tdektop(by, lis, False)
        elif self.query.data == "tran":
            data = get_tdektop(by, lis, True)
    ...
    async def handle_photo(self):
        ...
        # 如果记录已存在，执行 变更 picpath
        existing_user.callback_id = call_message.message_id
        logger.info(f"call_maessage.message_id为: {call_message.message_id}")
        logger.info(f"{self.update}")
        self.session.commit()
...
def get_de_modle(update, context, session, flag):
    model = DeAsyncModel(update, context, session, flag)
    machine = AsyncMachine(model, states=sta,
                           transitions=transition,
                           initial=sta[flag - off])
